...
public class Utils {
	...
	public static void writePerfResult(String module, long millis)
			throws IOException {
		PrintWriter writer = null;
		try {
			File outputFile = new File("target/test/perftime." + module
					+ ".txt");
			outputFile.delete();
	...
	public static long readPerfResult(String version, String module)
			throws IOException {
		BufferedReader reader = null;
		try {
			File inFile = new File("../" + version + "/target/test/perftime."
					+ module + ".txt");
	...
	public static File createTestFile(long largerThanMB) throws IOException {
		...
		FileUtils.copyFile(
				new File("../parquet-testdata/tpch/customer.schema"), new File(
						"target/test/csv/perftest.schema"));
		...
		final String[] versions = baseDir.list(new FilenameFilter() {
			public boolean accept(File dir, String name) {
				return name.startsWith("parquet-compat-")
						&& new Version(name.replace("parquet-compat-", ""))
	...
	static class Version implements Comparable<Version> {
		...
		Version(String versionStr) {
			...
			if (size > 4) {
				throw new RuntimeException("Illegal version number "
						+ versionStr);
			}
		}
		...
		public int compareMajorMinor(Version o) {
			return ComparisonChain.start().compare(major, o.major)
					.compare(minor, o.minor).result();
		}

		@Override
		public int compareTo(Version o) {
			return ComparisonChain.start().compare(major, o.major)
					.compare(minor, o.minor).compare(minorminor, o.minorminor)
					.compare(tag, o.tag).result();
		}
	...
	public static File getParquetOutputFile(String name, String module,
			boolean deleteIfExists) {
		File outputFile = new File("target/parquet/", getParquetFileName(name,
				module));
	...
	public static File getParquetFile(String name, String version,
			String module, boolean failIfNotExist) throws IOException {
		File parquetFile = new File("../" + version + "/target/parquet/",
				getParquetFileName(name, module));
		parquetFile.getParentFile().mkdirs();
		if (!parquetFile.exists()) {
			String msg = "File " + parquetFile.getAbsolutePath()
					+ " does not exist";
			if (failIfNotExist) {
	...
	public static String[] getImpalaDirectories() throws IOException {
		...
		final String[] impalaVersions = baseDir.list(new FilenameFilter() {
			public boolean accept(File dir, String name) {
				...
				return new Version(name).compareMajorMinor(new Version(
						currentVersion)) == 0;
			}
		});
	...
	public static File getParquetImpalaFile(String name, String impalaVersion)
			throws IOException {
		String fileName = name + ".impala.parquet";
		File parquetFile = new File("../parquet-testdata/impala/"
				+ impalaVersion, fileName);
		if (!parquetFile.exists()) {
	...
	public static File getCsvTestFile(String name, String module,
			boolean deleteIfExists) {
		File outputFile = new File("target/test/csv/", name
				+ (module != null ? "." + module : "") + ".csv");
		outputFile.getParentFile().mkdirs();
		if (deleteIfExists) {
			outputFile.delete();
		}
	...
	public static File getParquetTestFile(String name, String module,
			boolean deleteIfExists) {
		File outputFile = new File("target/test/parquet/", name
				+ (module != null ? "." + module : "") + ".csv");
		outputFile.getParentFile().mkdirs();
		if (deleteIfExists) {
			outputFile.delete();
	...
	public static void verify(File expectedCsvFile, File outputCsvFile)
			throws IOException {
		...
		try {
			...
			String lineOut = null;
			int lineNumber = 0;
			while ((lineIn = expected.readLine()) != null
					&& (lineOut = out.readLine()) != null) {
				++lineNumber;
	...
	public static void verify(File expectedCsvFile, File outputCsvFile,
			boolean orderMatters) throws IOException {
		if (!orderMatters) {
